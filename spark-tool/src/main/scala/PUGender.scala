...
import org.apache.spark.ml.classification.PULogisticRegression
...
object PUGender {
  def main(args: Array[String]): Unit = {
    ...
    val dataset = spark.read.format("csv")
      ...
      .select(labelUDF($"label").as("tag"),
        gdf(labelUDF($"label")).as("label"),
      ...
      .filter("size(applist)>10 and size(applist)<200")
    //                .withColumn("weight", wdf($"label"))
    //                .sample(0.01)
    ...
    val lr = new PULogisticRegression()
    ...
    // feature Transform
    val Array(trainDataset, testDataset) = featureEncoder.transform(dataset).randomSplit(Array(0.7, 0.3))
    trainDataset.cache
    testDataset.cache
    ...
    val output_t = finalModel.transform(trainDataset)
      .select($"lr_p", $"tag")
      .as[(Double, Double)].rdd
